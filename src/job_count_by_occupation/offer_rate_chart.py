...
RATE_TYPES = {
    "vs_hellowork": "job_offer_count / prefecture_hellowork_job_count",
    "vs_base": "job_offer_count / prefecture_base_job_count",
}


def create_sample_offer_rate_csv(output_csv: Path) -> Path:
    rows = [
        ["2025-11", "北海道", "サービス", "介護サービス職業従事者", 1200, 2200, 180],
        ["2025-12", "北海道", "サービス", "介護サービス職業従事者", 1180, 2180, 176],
        ["2026-01", "北海道", "サービス", "介護サービス職業従事者", 1250, 2300, 190],
        ["2026-02", "北海道", "サービス", "介護サービス職業従事者", 1280, 2360, 196],
        ["2025-11", "東京都", "サービス", "介護サービス職業従事者", 5200, 9100, 610],
        ["2025-12", "東京都", "サービス", "介護サービス職業従事者", 5180, 9050, 598],
        ["2026-01", "東京都", "サービス", "介護サービス職業従事者", 5350, 9340, 625],
        ["2026-02", "東京都", "サービス", "介護サービス職業従事者", 5480, 9580, 642],
        ["2025-11", "北海道", "専門・技術", "保健師，助産師，看護師", 980, 1840, 152],
        ["2025-12", "北海道", "専門・技術", "保健師，助産師，看護師", 995, 1865, 158],
        ["2026-01", "北海道", "専門・技術", "保健師，助産師，看護師", 1020, 1920, 163],
        ["2026-02", "北海道", "専門・技術", "保健師，助産師，看護師", 1045, 1970, 171],
        ["2025-11", "東京都", "専門・技術", "保健師，助産師，看護師", 4100, 7600, 520],
        ["2025-12", "東京都", "専門・技術", "保健師，助産師，看護師", 4150, 7680, 528],
        ["2026-01", "東京都", "専門・技術", "保健師，助産師，看護師", 4230, 7800, 539],
        ["2026-02", "東京都", "専門・技術", "保健師，助産師，看護師", 4310, 7940, 548],
    ]
...
def generate_offer_rate_explorer_html(input_csv: Path, output_html: Path) -> Path:
    dataset = _load_offer_rate_dataset(input_csv)
    html_text = _build_offer_rate_explorer_html(dataset)
    output_html.parent.mkdir(parents=True, exist_ok=True)
    output_html.write_text(html_text, encoding="utf-8")
    return output_html


def _load_offer_rate_dataset(input_csv: Path) -> Dict[str, object]:
    dates: List[str] = []
    prefectures: List[str] = []
    major_categories: List[str] = []
    occupations: List[str] = []
    date_codes: Dict[str, int] = {}
    ...
    with input_csv.open(encoding="utf-8") as fh:
        ...
        required = {"date", "prefecture", "major_category", "occupation_name", "prefecture_base_job_count", "job_offer_count"}
        missing = required - set(reader.fieldnames or [])
        if missing:
            raise ValueError(f"入力CSVに必要な列がありません: {', '.join(sorted(missing))}")

        hellowork_column = None
        for candidate in ["prefecture_hellowork_job_count", "prefecture_hellowork_jo_count"]:
            if candidate in (reader.fieldnames or []):
                hellowork_column = candidate
                break
        if hellowork_column is None:
            raise ValueError("入力CSVに prefecture_hellowork_job_count 列がありません。")
        ...
        for row in reader:
            ...
            rows.append(
                [
                    encode(date, date_codes, dates),
                    encode(prefecture, prefecture_codes, prefectures),
                    encode(major_category, major_codes, major_categories),
                    encode(occupation_name, occupation_codes, occupations),
                    hellowork,
                    base,
                    offer,
                ]
    ...
    order = sorted(range(len(dates)), key=lambda idx: dates[idx])
    remap = {old: new for new, old in enumerate(order)}
    ordered_dates = [dates[idx] for idx in order]
    for row in rows:
        row[0] = remap[row[0]]
...
def _build_offer_rate_explorer_html(dataset: Dict[str, object]) -> str:
...
<body>
  <main class="wrap">
    <h1>応募率の時系列探索</h1>
    <p class="lead">入力CSVから `job_offer_count / prefecture_hellowork_job_count` と `job_offer_count / prefecture_base_job_count` を計算し、都道府県別・大分類別・職種別・都道府県×職種別の折れ線グラフを切り替えて見られる HTML です。</p>
    <section class="panel">
      <div class="controls">
        <label>表示モード
          <select id="viewMode">
            ...
            <option value="prefecture_occupation">prefecture & occupation_name別</option>
          </select>
        </label>
        <label>rate
      ...
      <div class="hint">
        比較しない軸はフィルタとして使います。例えば `occupation_name別` で `prefecture=北海道`、`major_category=サービス` を選ぶと、北海道のサービス職種だけを比較します。
      </div>
    </section>
  </main>
  <script id="rate-data" type="application/json">{dataset_json}</script>
  <script>
    const DATA = JSON.parse(document.getElementById("rate-data").textContent);
    const ALL = "__all__";
    const COLORS = ["#0f766e","#2563eb","#dc2626","#7c3aed","#d97706","#059669","#db2777","#4f46e5","#0891b2","#65a30d"];
    ...
    function init() {{
      setOptions(rateTypeEl, Object.entries(DATA.rateTypes), false);
      setOptions(prefectureEl, DATA.prefectures.map((value) => [value, value]), true, "全prefecture");
      setOptions(majorEl, DATA.majorCategories.map((value) => [value, value]), true, "全major_category");
      setOptions(occupationEl, DATA.occupations.map((value) => [value, value]), true, "全occupation_name");
      rateTypeEl.value = "vs_hellowork";
      for (const el of [viewModeEl, rateTypeEl, prefectureEl, majorEl, occupationEl, topNEl]) {{
    ...
    function render() {{
      ...
      chartWrapEl.innerHTML = renderSvg(series, DATA.dates, 1260, 520);
      legendEl.innerHTML = series.map((item, index) => `
        <div class="legend-item">
          <span class="swatch" style="background:${{COLORS[index % COLORS.length]}}"></span>
          <span><strong>${{escapeHtml(item.name)}}</strong> 最新: ${{item.latest.toFixed(4)}}</span>
        </div>
      `).join("");
    }}
    ...
    function buildSeries(state) {{
      ...
      for (const row of DATA.rows) {{
        const date = DATA.dates[row[0]];
        const prefecture = DATA.prefectures[row[1]];
        const major = DATA.majorCategories[row[2]];
        const occupation = DATA.occupations[row[3]];
        ...
        if (state.view === "prefecture") {{
          ...
          if (state.prefecture !== ALL && prefecture !== state.prefecture) continue;
          if (state.occupation !== ALL && occupation !== state.occupation) continue;
          key = `${{prefecture}}__${{occupation}}`;
          label = `${{prefecture}} / ${{occupation}}`;
        }}
    ...
    function renderSvg(series, labels, width, height) {{
      ...
      const pad = Math.max(0.01, (maxValue - minValue) * 0.08);
      ...
      for (let step = 0; step < 6; step += 1) {{
        ...
        grid.push(`<text x="${{left-10}}" y="${{(y+4).toFixed(2)}}" text-anchor="end" font-size="12" fill="#57534e">${{value.toFixed(3)}}</text>`);
      }}
      ...
      series.forEach((item, index) => {{
        ...
        const filtered = item.points.map((value, pointIndex) => [value, pointIndex]).filter(([value]) => value !== null);
        const path = filtered.map(([value, pointIndex]) => `${{xAt(pointIndex).toFixed(2)}},${{yAt(value).toFixed(2)}}`).join(" ");
        lines.push(`<polyline fill="none" stroke="${{color}}" stroke-width="2.5" points="${{path}}" />`);
        const last = filtered[filtered.length - 1];
        const endX = xAt(last[1]);
        const endY = yAt(last[0]);
